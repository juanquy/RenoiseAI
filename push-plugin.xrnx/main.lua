...
C.LED.WHITE         = 3 -- Bright Amber Stage (Visible on all Push 1)
...
local function update_button_leds()
  ...
  -- Push 1 CC Buttons: 4 is Bright, 1 is Dim/Backlight.
  -- 17-19 (Green) ONLY work on Transport/Pads.
  Midi.set_button_led(C.BTN.SESSION, active_up == renoise.ApplicationWindow.UPPER_FRAME_PATTERN_EDITOR and 4 or 1)
...
local function handle_navigation(id, val)
  ...
  if id == C.BTN.UP then
    pcall(function() if mode == "matrix" then Matrix.scroll_seq(-1) end end)
    pcall(function()
      if shift_held then song.selected_sequence_index = math.max(1, song.selected_sequence_index - 1)
      else song.selected_line_index = math.max(1, song.selected_line_index - 1) end
    end)
  elseif id == C.BTN.DOWN then
    pcall(function() if mode == "matrix" then Matrix.scroll_seq(1) end end)
    pcall(function()
      if shift_held then song.selected_sequence_index = math.min(#song.sequencer.pattern_sequence, song.selected_sequence_index + 1)
      else song.selected_line_index = song.selected_line_index + 1 end
    end)
  elseif id == C.BTN.LEFT then
    pcall(function() if mode == "matrix" then Matrix.scroll_tracks(-1) end end)
    pcall(function()
      if shift_held then song.selected_track_index = math.max(1, song.selected_track_index - 8)
      else song.selected_track_index = math.max(1, song.selected_track_index - 1) end
    end)
  elseif id == C.BTN.RIGHT then
    pcall(function() if mode == "matrix" then Matrix.scroll_tracks(1) end end)
    pcall(function()
      if shift_held then song.selected_track_index = math.min(#song.tracks, song.selected_track_index + 8)
      else song.selected_track_index = song.selected_track_index + 1 end
    end)
  end
  update_display()
end
...
local function on_midi_message(msg)
  ...
  if pressed then
    local debug_msg = ""
    if msg.is_pad then
      debug_msg = string.format("[AG-DEBUG] PUSH PAD: Note %d (Val: %d)", btn_id or 0, val)
    else
      debug_msg = string.format("[AG-DEBUG] PUSH CC: %d (Value: %d)", btn_id or 0, val)
    end
...
local function process_action(a)
  ...
  -- TRANSPORT PRIORITY
  if a.btn_id == C.BTN.PLAY or a.btn_id == C.BTN.RECORD or a.btn_id == C.BTN.TAP_TEMPO then 
...
local function tool_tick()
  ...
  for i = 1, #current_actions do
    local ok, err = pcall(process_action, current_actions[i])
    if not ok then print("[Push Error] process_action failed: " .. tostring(err)) end
  end
  ...
  pcall(function()
    ...
    if mode == "matrix" then 
      local ok, err = pcall(function() Matrix.render() end)
      if not ok then show_mode_notification("MATRIX RENDER ERROR: " .. tostring(err)) end
    elseif pos.line ~= last_line then
      last_line = pos.line
  ...
  -- 4. Flush Hardware State
  pcall(function() Midi.flush_hardware() end)
