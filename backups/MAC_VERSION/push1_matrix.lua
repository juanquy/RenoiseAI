    ...
    if seq_idx > num_seq then
      ...
      for col = 0, 7 do
        ...
        if trk_idx <= num_tracks then
          ...
          local is_muted = song.sequencer:track_sequence_slot_is_muted(trk_idx, seq_idx)
...
function M.handle_pad_press(row, col)
  ...
  if seq_idx <= #song.sequencer.pattern_sequence and trk_idx <= song.sequencer_track_count then
    ...
    local was_muted = song.sequencer:track_sequence_slot_is_muted(trk_idx, seq_idx)
    song.sequencer:set_track_sequence_slot_is_muted(trk_idx, seq_idx, not was_muted)
  end
