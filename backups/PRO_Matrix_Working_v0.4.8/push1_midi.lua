...
function M.find_push_ports()
  ...
  -- GHOST FILTER: Ignore any port created by Renoise itself to find the TRUE hardware
  for _, n in ipairs(in_devs) do
    if n:lower():find("push") and n:lower():find("user") and not n:lower():find("renoise") then in_p = n; break end
  end
  for _, n in ipairs(out_devs) do
    if n:lower():find("push") and n:lower():find("user") and not n:lower():find("renoise") then out_p = n; break end
  end
...
function M.find_all_push_outputs()
  ...
  -- Priority 1: Ports with "user", "midi 2", or "port 2" in the name
  for _, n in ipairs(out_devs) do
    if n:lower():find("push") and (n:lower():find("user") or n:lower():find("midi 2") or n:lower():find("port 2")) and not n:lower():find("renoise") then
      table.insert(targets, n)
    end
  end
  
  -- Priority 2: Any other Push ports
  if #targets == 0 then
    for _, n in ipairs(out_devs) do
      if n:lower():find("push") and not n:lower():find("renoise") then
        table.insert(targets, n)
      end
    end
  end
  
  -- If we found two, and neither had "user" in the name, prefer the second one (usually the User Port)
  if #targets == 2 and not targets[1]:lower():find("user") and not targets[2]:lower():find("user") then
    return {targets[2], targets[1]} -- Prefer the 2nd one
  end
...
function M.connect(manual_in, manual_out)
  ...
  -- Force dual-input discovery for Linux: find ALL Push inputs
  local all_ins = renoise.Midi.available_input_devices()
  local target_ins = {}
  
  if manual_in then 
    target_ins = {manual_in} 
  else
    for _, n in ipairs(all_ins) do
      if n:lower():find("push") then table.insert(target_ins, n) end
    end
  end

  local out_targets = (manual_out) and {manual_out} or M.find_all_push_outputs()
  if #target_ins == 0 or #out_targets == 0 then return false, "Push 1 hardware not found." end

  M.disconnect()

  local status_parts = {}
  for _, in_name in ipairs(target_ins) do
    ...
    if ok then table.insert(status_parts, in_name:sub(-10)) end
  end
...
function M._on_raw_midi(message, port_name)
  ...
  -- DIRECT BYPASS for Transport (High Priority)
  if msg_type == 0xB0 and (data1 == 85 or data1 == 86) then
    if data2 > 0 then 
    ...
    return -- STOP processing here so it doesn't double-trigger in the queue
  end
...
function M.flush_hardware()
  if #M.midi_outputs == 0 then return end
  local sent_count = 0
  local MAX_SENT = 80 -- Stabilized for Linux/ALSA (Pads: 64, Buffers: safe)

  -- 1. Flush BUTTON LEDs (High Priority)
  for cc, color in pairs(btn_buffer) do
    if sent_count >= MAX_SENT then break end
    if btn_state[cc] ~= color then
      btn_state[cc] = color
  ...
  for i = 1, 4 do
    if sent_count >= MAX_SENT then break end
    if display_buffer[i] ~= display_state[i] then
